...
- 進行中: `- [ ] Task 1.2` → `- [ ] Task 1.2 🚧`
- ブロック中: `- [ ] Task 1.3` → `- [ ] Task 1.3 ⚠️`
...
- **動作**:
  1. 完了したタスクの内容を確認する。
  2. `.kiro/specs/bookmark-manager/tasks.md` の該当タスクに `✅` マークを追加する。
...
- **コミットメッセージ**: `[Task X.Y] 機能名: 実装内容`
- **進捗マーク**: `[Task X.Y completed]` で自動進捗更新
- **テスト**: 各タスク完了時に必ずテスト実行
